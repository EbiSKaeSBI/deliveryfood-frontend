...
export const getPartners = async (query: string, page: number = 1): Promise<PartnersResponse> => {
  return await api.get(`partners?search=${query}&page=${page}`).then((response) => response.data);
};
