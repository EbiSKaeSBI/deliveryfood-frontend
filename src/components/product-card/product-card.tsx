...
const ProductCard = ({product}:ProductCardProps) => {
    return (
        <div className="bg-white shadow-[0_4px_12px_rgba(0,0,0,0.05)] rounded-[7px] overflow-hidden mb-[30px] basis-[31%] no-underline">
            <img src={product.image} alt="" className="w-full h-[250px] object-cover"/>
            <div className="pt-[20px] pr-[23px] pb-[35px] pl-[23px] min-h-[275px] flex flex-col">
                <div className="flex items-center justify-between mb-[10px]">
                    <h3>{product.name}</h3>
                </div>
                <div className="flex items-center flex-wrap">
                    <div className="text-[#8c8c8c] text-[18px]/[21px]">{product.description}</div>
                </div>
                <div className="flex items-center flex-wrap mt-auto">
                    ...
                    <strong className="font-bold text-[20px]/[32px] ml-[30px]">{product.price} ₽</strong>
                </div>
            </div>
        </div>
    );
